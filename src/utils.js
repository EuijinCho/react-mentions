...
export var applyChangeToValue = function applyChangeToValue(value, markup, plainTextValue, selectionStartBeforeChange, selectionEndBeforeChange, selectionEndAfterChange, displayTransform, regex) {
    ...
    if (window.navigator.platform.indexOf("Mac") !== -1 && selectionStartBeforeChange === selectionEndBeforeChange && selectionEndBeforeChange === selectionEndAfterChange && oldPlainTextValue.length === plainTextValue.length) {
    ...
    if (isWinChrome && selectionStartBeforeChange < selectionEndAfterChange) {
        spliceEnd = selectionEndAfterChange + lengthDelta;
    }
