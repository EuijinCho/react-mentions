...
var MentionsInput = (_temp = _class = function (_React$Component) {
    ...
    this.getInputProps = function (isTextarea) {
        ...
        return _extends({}, props, style('input'), {
            ...
            // onKeyPress: function(e) {if (!!document.documentMode) {_this3.handleSelect(e)}},
            onChangeCapture: function(e) {_this3.handleSelect(e)},
    ...
    this.handleChange = function (ev) {
        ...
        var isWinChrome = window.navigator.platform.indexOf("Win") !== -1 && window.navigator.appVersion.indexOf("Chrome") !== -1;
        var checkKoreanChar = /[ㄱ-ㅎㅏ-ㅣ\uAC00-\uD7AF]/;
        var isWinChromeKoreanChar = false;
        if (isWinChrome) {
            var lastIndex = newPlainTextValue[_this3.state.selectionStart] ?
                _this3.state.selectionStart : _this3.state.selectionStart-1;
            var lastChar = newPlainTextValue[lastIndex];
            isWinChromeKoreanChar = checkKoreanChar.test(lastChar);
        }

        var convertedSelectionEnd = isWinChromeKoreanChar ? ev.target.selectionEnd + 1 : ev.target.selectionEnd;
        ...
        var newValue = applyChangeToValue(value, markup, newPlainTextValue, _this3.state.selectionStart, _this3.state.selectionEnd, convertedSelectionEnd, displayTransform, regex);
        ...
        var selectionStart = convertedSelectionEnd;
        var selectionEnd = convertedSelectionEnd;
    ...
    this.handleSelect = function (ev) {
        if (ev.nativeEvent.key === "Tab" || ev.nativeEvent.key === "Enter") {
            return;
        }
        ...
        var isWinPlatform = window.navigator.platform.indexOf("Win") !== -1;
        if (!isWinPlatform && ev.target.selectionStart !== ev.target.selectionEnd) {
            _this3.clearSuggestions();
        }
        else {
            _this3.updateMentionsQueries(el.value, ev.target.selectionStart, isWinPlatform);
        }
    ...
    this.updateMentionsQueries = function (plainTextValue, caretPosition,isWinPlatform) {
        ...
        var checkKoreanChar = /[ㄱ-ㅎ\uAC00-\uD7AF]/;
        var isLastCharKorean = checkKoreanChar.test(plainTextValue[caretPosition]);
        var v = isWinPlatform && isLastCharKorean ? plainTextValue : value;
        var positionInValue = mapPlainTextIndex(v, markup, caretPosition, 'NULL', displayTransform, regex);
        // var positionInValue = mapPlainTextIndex(value, markup, caretPosition, 'NULL', displayTransform, regex);
        positionInValue = isWinPlatform && isLastCharKorean ? positionInValue + 1 : positionInValue;
        ...
        var cp = isWinPlatform && isLastCharKorean ? caretPosition + 1 : caretPosition;
        var substring = plainTextValue.substring(substringStartIndex, cp);
        ...
        React.Children.forEach(children, function (child) {
            ...
            if (match) {
                ...
                var querySequenceEnd = isWinPlatform && isLastCharKorean ? querySequenceStart + match[1].length + 1 : querySequenceStart + match[1].length;
                _this3.queryData(match[2], child, querySequenceStart, querySequenceEnd, plainTextValue);
